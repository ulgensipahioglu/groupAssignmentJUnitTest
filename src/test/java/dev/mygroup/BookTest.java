...
import static org.junit.Assert.assertFalse; // all methots imported
...
public class BookTest {
    ...
    public void testCheckLateFeeOnlyAfterSevenDays() {
        ...
        for (int i = 0; i < 7; i++) {
            book.advanceDay(); // 5 days borrowed -> should be free

        }
        assertEquals(0, book.checkLateFee());

        for (int i = 0; i < 3; i++) {
            book.advanceDay(); // 8 days borrowed -> 1 day late -> 20 kr fee

        }
        assertEquals(20, book.checkLateFee());

        for (int i = 0; i < 2; i++) {
            book.advanceDay(); // 10 days borrowed -> 3 days late -> 60 kr fee
    ...
    public void testExtendTimeIncreasesDaysBorrowed() {
        Book book = new Book("TestBook", "Fiction", "Tester");
        for (int i = 0; i < 10; i++) {
            book.advanceDay();     // 10 days borrowed
        }
